...
(function () {
  ...
  Cb.start = async (initial) => {
    ...
    log('Turn order: ' + Cb.order.map((e) => e.name + ' (' + e.initiative + ')').join(', ') + '.', 'roll');
  ...
  Cb.addToOrder = (e, resort, quiet) => {
    ...
    Cb.order.push(e); if (resort) { Cb.sortOrder(); log(e.name + ' rolls initiative: ' + Dice.fmt(rec), 'roll'); }
  };
  ...
  Cb.join = (m) => { if (!Cb.active || Cb.participants.has(m.id)) return; log(m.name + ' joins the fight!', 'warn'); Cb.addToOrder(m, true, true); Game.refreshHud(); }; // a joiner's initiative is logged, not thrown over whatever is on the table
  ...
  Cb.run = async () => {
    while (Cb.active) {
      ...
      Game.refreshHud();
      if (e.mon) { UI.turnBanner(e.name + "'s turn"); await Cb.monsterTurn(e); UI.turnBanner(null); if (!Cb.active) return; continue; }
      // Party member
  ...
  Cb.attack = async (att, tgt, opts) => {
    ...
    Game.showRoll(rec);
    await UI.waitForDice(); // the roll lands and reads out before the hit or miss is resolved: one thing at a time
    ...
    if (isMon) {
      ...
      if (atk.extra) { if (atk.extra.save) { const s = Rules.savingThrow(tgt, atk.extra.save, atk.extra.dc, { vsPoison: atk.extra.type === 'poison', label: U.cap(atk.extra.type) + ' save' }); await UI.waitForDice(); const r2 = Dice.roll(atk.extra.dmg, { silent: true }); let ed = s.success ? (atk.extra.half ? Math.floor(r2.total / 2) : 0) : r2.total; if (ed > 0) extras.push({ dmg: ed, type: atk.extra.type }); } else { const r2 = Dice.roll(atk.extra.dmg, { crit, silent: true }); extras.push({ dmg: r2.total, type: atk.extra.type }); } }
  ...
  Cb.castSpell = async (caster, spellId, target, opts) => {
    ...
    let dmgRoll = null; if (spell.damage && spell.damage !== '0') { let expr = scaleExpr(spell.damage); if (!isMon && caster.cls === 'wizard' && caster.level >= 10 && spell.school === 'evocation') expr += '+' + mod; dmgRoll = Dice.roll(expr, { label: spell.name + ' damage', kind: 'dmg' }); await UI.waitForDice(); }
