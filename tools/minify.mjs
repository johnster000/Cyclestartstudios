...
// Visitor counting. The game pages are copies pulled from each game's own repo, so a
// tag pasted into one is lost on the next sync - it belongs here, where every published
// page is stamped. GoatCounter sets no cookies, stores no personal data and needs no
// consent banner; count.js skips localhost on its own, so local runs never count.
const ANALYTICS =
  '<script data-goatcounter="https://cyclestartstudios.goatcounter.com/count"' +
...
async function run(gameDir) {
  ...
  for (const f of htmlFiles) {
    ...
    out = out.replace(/<\/body>/i, () =>
      `<script>document.documentElement.dataset.csx="${canary}"</script>` +
      ANALYTICS + '</body>');
    after += out.length;
